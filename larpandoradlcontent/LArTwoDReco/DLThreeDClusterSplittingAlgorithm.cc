...
StatusCode DLThreeDClusterSplittingAlgorithm::Run()
{
    ...
    {
        const ClusterList *pClusterList(hitType == TPC_VIEW_U ? m_pClusterListU : hitType == TPC_VIEW_V ? m_pClusterListV : m_pClusterListW);
        ...
        for (const Cluster *const pCluster : internalClusterVector)
        {
            ...
            this->ProcessCluster(pCluster,
                                 (hitType == TPC_VIEW_U ? kdTreeU : hitType == TPC_VIEW_V ? kdTreeV : kdTreeW),
                                 foundSplitPositions);
    ...
    // for (const HitType hitType : {TPC_VIEW_U, TPC_VIEW_V, TPC_VIEW_W})
    // {
    //     const CartesianPointVector &split(hitType == TPC_VIEW_U ? splitPosU : hitType == TPC_VIEW_V ? splitPosV : splitPosW);
    //     for (const CartesianVector &jam : split)
    //     {
    //         CartesianVector pos(jam);
    //         PandoraMonitoringApi::AddMarkerToVisualization(this->GetPandora(), &pos, "split", BLUE, 2);
    //     }
    // }
    ...
    this->ThreeViewMatching(splitPosU, splitPosV, splitPosW, usedU, usedV, usedW); // With all three views
    ...
    //     for (int usedI : used)
    //     {
    //         CartesianVector pos(split.at(usedI));
    //         PandoraMonitoringApi::AddMarkerToVisualization(this->GetPandora(), &pos, "acceptedSplit", RED, 2);
    //     }
    // }
    // PandoraMonitoringApi::ViewEvent(this->GetPandora());
    ////////////////////////////////////////////////////////////////
    ...
    for (const HitType hitType : {TPC_VIEW_U, TPC_VIEW_V, TPC_VIEW_W})
    {
        ...
        for (const auto& [pCluster, splitIndices] : clusterToSplitIndex) 
        {
            ...
            // Divide calo hits
            std::vector<CaloHitList> splitClusterHits(this->DivideCaloHits(clusterHits, clusterFit, foundSplitPositions));
...
void DLThreeDClusterSplittingAlgorithm::ProcessCluster(const Cluster *const pCluster, HitKDTree2D &kdTree, CartesianPointVector &splitPositions)
{
    // Enough hits?
    CaloHitList clusterHits;
    LArClusterHelper::GetAllHits(pCluster, clusterHits);
    ...
    try
    {
        ...
        if (m_trainingMode)
        {
            ...
            this->FindSplitPositions(clusterFit, mcContaminants, splitPositions);
    ...
    HitKDTree2D &kdTree) const
{
    ...
    for (unsigned int i = 0; i < clusterPath.size(); ++i)
    {
        ...
        features.at("SecVertex").m_sequence.push_back(
            this->GetDistanceToSecVertex(pPathCaloHit));
...
float DLThreeDClusterSplittingAlgorithm::GetDistanceToSecVertex(const CaloHit *const pCaloHit) const
{
    if (!m_pSecVertexList)
        return -1.f;
...
float DLThreeDClusterSplittingAlgorithm::GetDistanceToGap(const CaloHit *const pPrevHit, const CaloHit *const pCurrentHit) const
{
    if (!pPrevHit)
        return 1.f;
...
void DLThreeDClusterSplittingAlgorithm::GetSplitIndices(const Features &features, IntVector &splitIndices)
{
...
void DLThreeDClusterSplittingAlgorithm::GetSplitIndices(const Features &features, const IntVector &windowStart, IntVector &splitIndices, 
    FloatVector &splitScores)
{
    ...
    for (unsigned int i = 0; i < windowStart.size(); ++i)
    {
        ...
        // Make sure that we're not too showery (unless its the first/last window)
        // if ((i != 0) && (i != (windowStart.size() - 1)) && (shrProb_new > 0.1))
        //     continue;
        ...
        if (sigProb_new > m_isContaminatedThreshold)
        {
            ...
            for (unsigned int j = 0; j < m_windowLength; ++j)
            {
                ...
                {
                    // Double-check that we are not near a gap
                    bool nearGap(false);
                    for (int k = -2; k < 3; ++k)
                    {
                        int newIndex(windowStart.at(i) + j + k);

                        if (newIndex >= 0 && newIndex < int(features.at("GapSep").m_sequence.size()))
                        {
                            if (features.at("GapSep").m_sequence.at(newIndex) < std::numeric_limits<float>::epsilon())
                            {
                                nearGap = true;
                                break;
                            }
                    ...
                    //std::cout << "sigProb: " << sigProb_new << " shrProb: " << shrProb_new << " splitProb: " << splitProb << std::endl;
...
void DLThreeDClusterSplittingAlgorithm::FilterSplitIndices(const ClusterPath &clusterPath, const HitType hitType, IntVector &splitIndices) const
{
    ...
    for (unsigned int iU = 0; iU < splitPosU.size(); ++iU)
    {
        ...
        for (unsigned int iV = 0; iV < splitPosV.size(); ++iV)
        {
            ...
            for (unsigned int iW = 0; iW < splitPosW.size(); ++iW)
            {
                ...
                const float predW(LArGeometryHelper::MergeTwoPositions(this->GetPandora(), TPC_VIEW_U, TPC_VIEW_V, splitPosU.at(iU).GetZ(), splitPosV.at(iV).GetZ()));
                const float predV(LArGeometryHelper::MergeTwoPositions(this->GetPandora(), TPC_VIEW_U, TPC_VIEW_W, splitPosU.at(iU).GetZ(), splitPosW.at(iW).GetZ()));
                const float predU(LArGeometryHelper::MergeTwoPositions(this->GetPandora(), TPC_VIEW_V, TPC_VIEW_W, splitPosV.at(iV).GetZ(), splitPosW.at(iW).GetZ()));
    ...
    const HitType hitType1, const HitType hitType2, HitKDTree2D &kdTree, IntVector &used1, IntVector &used2)
{
    for (unsigned int i1 = 0; i1 < splitPos1.size(); ++i1)
    {
        ...
        for (unsigned int i2 = 0; i2 < splitPos2.size(); ++i2)
        {
            ...
            if (std::fabs(splitPos1.at(i1).GetX() - splitPos2.at(i2).GetX()) > 0.5f)
                continue;
            ...
            // Collect close hits
            HitKDNode2DList foundHits;
            KDTreeBox searchRegionHits(build_2d_kd_search_region(predPosition3, 0.5f, 0.5f));
            kdTree.search(searchRegionHits, foundHits);

            if (!foundHits.empty() && (foundHits.size() < 3))
            {
                used1.push_back(i1);
                used2.push_back(i2);
            }
...
std::vector<CaloHitList> DLThreeDClusterSplittingAlgorithm::DivideCaloHits(const CaloHitList &clusterHits, const TwoDSlidingFitResult &clusterFit,
...
void DLThreeDClusterSplittingAlgorithm::FindSplitPositions(const TwoDSlidingFitResult &clusterFit, const MCContaminantVector &mcContaminants,
    CartesianPointVector &trueSplitPositions)
{
    float m_endpointBuffer(2.f);
