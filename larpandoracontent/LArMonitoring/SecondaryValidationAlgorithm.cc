...
StatusCode SecondaryValidationAlgorithm::Run()
{
    ...
    // Get # of reconstructable particles
    int nReconstructableParticles(0);

    for (const LArHierarchyHelper::MCMatches &mcMatch : mcMatches)
    {
        const LArHierarchyHelper::MCHierarchy::Node *const pMCNode(mcMatch.GetMC());

        if (pMCNode->IsReconstructable())
            ++nReconstructableParticles;
    }
    ...
    for (const LArHierarchyHelper::MCMatches &mcMatch : mcMatches)
    {
        ...
        this->FillTree(pPrimaryNode, pPrimaryNode->GetMCParticles().front(), mcMatch.GetRecoMatches().front(), hierarchyNodes, mcMatches, 3, nReconstructableParticles);
    }
...
void SecondaryValidationAlgorithm::FillTree(const LArHierarchyHelper::MCHierarchy::Node *const pParentMCNode,
    ...
    const LArHierarchyHelper::MCMatchesVector mcMatches, const int tierToExamine, const int nRecoParticles)
{
    ...
    {        
        ...
        {   
            this->FillTree(pParentMCNode, pMCChild, pParentRecoNode, hierarchyNodes, mcMatches, tierToExamine, nRecoParticles);
            continue;
        }
        ...
        {        
            ...
            this->FillEntry(pParentMCNode, pChildMCNode,  pChildMatch, mcMatches, tierToExamine, nRecoParticles);            
            this->FillTree(pChildMCNode, pMCChild, pChildMatch->GetRecoMatches().empty() ? nullptr : pChildMatch->GetRecoMatches().front(), hierarchyNodes, mcMatches, (tierToExamine + 1), nRecoParticles);
        }
    }
}
...
void SecondaryValidationAlgorithm::FillEntry(const LArHierarchyHelper::MCHierarchy::Node *const pParentMCNode,
    ...
    const LArHierarchyHelper::MCMatchesVector &matchesVector, const int hierarchyTier, const int nRecoParticles)
{
